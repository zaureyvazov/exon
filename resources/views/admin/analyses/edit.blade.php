...
@section('title', 'Analiz Redəktə Et')
...
@section('content')
    <div class="mb-4">
        <h1 class="fw-bold text-dark mb-2"><i class="bi bi-pencil-square"></i> Analiz Redəktə Et</h1>
        <p class="text-muted">Analiz məlumatlarını dəyişdirin</p>
    </div>
        ...
        <div class="col-12 col-lg-10 col-xl-8">
            <div class="card border-0 shadow-sm">
                <div class="card-body p-4">
                    <form action="{{ route('admin.analyses.update', $analysis->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            >
                                ...
                                @foreach(\App\Models\AnalysisCategory::orderBy('name')->get() as $category)
                                    <option value="{{ $category->id }}"
                                            {{ old('category_id', $analysis->category_id) == $category->id ? 'selected' : '' }}
                                            @if(!$category->is_active) style="color: #dc3545;" @endif>
                                        {{ $category->name }}{{ !$category->is_active ? ' (Deaktiv)' : '' }}
                                    </option>
                                @endforeach
                            </select>
                            @error('category_id')
                        ...
                        <div class="mb-3">
                            ...
                            >{{ old('description', $analysis->description) }}</textarea>
                            @error('description')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <input
                                    ...
                                    class="form-control"
                                    value="{{ old('commission_percentage', $analysis->commission_percentage) }}"
                                    step="0.01"
                                    min="0"
                                    max="100"
                                    required
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <input
                                    ...
                                    class="form-control"
                                    value="{{ old('discount_commission_rate', $analysis->discount_commission_rate ?? 0) }}"
                                    step="0.01"
                                    min="0"
                                    max="100"
                                    required
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input
                                    ...
                                    name="is_active"
                                    value="1"
                                    {{ old('is_active', $analysis->is_active) ? 'checked' : '' }}
                                >
